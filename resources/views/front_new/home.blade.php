...
@php
    ...
    if ($topAd) {
        $adImageDesktopTop = getAdImageDesktop(\App\Models\AdSpaces::INDEX_TOP);
        $adImageMobileTop = getAdImageMobile(\App\Models\AdSpaces::INDEX_TOP);
    }

    if ($bottomAd) {
        $adImageDesktopBottom = getAdImageDesktop(\App\Models\AdSpaces::INDEX_BOTTOM);
        $adImageMobileBottom = getAdImageMobile(\App\Models\AdSpaces::INDEX_BOTTOM);
    }
...
@section('content')
    <div class="home-page">
                        ...
                        <div
                            ...
                            <div class="container">
                                <div class="hero-content position-absolute">
                                    <h1 class="pb-2"><a href="{{ route('detailPage',$sliderPost->slug )}}"
                                                        class="text-decoration-none">{!! \Illuminate\Support\Str::limit($sliderPost->title,85,'...') !!}</a>
                                    </h1>
                                    <div class="desc d-sm-flex align-items-center justify-content-between">
                                        <div class="desc d-flex align-items-center">
                                            <a href="{{route('categoryPage',$sliderPost->category->slug)}}"
                                               class="tags {{getColorClass($sliderPost->category->id)}}">{!! $sliderPost->category->name !!}</a>
                                            <p class="fs-14 date mb-0">{{ ucfirst(__('messages.common.'.strtolower($sliderPost->created_at->format('F')))) }} {{ $sliderPost->created_at->format('d, Y') }}</p>
                                            <span class=" text-primary px-sm-4 px-2"> | </span>
                                            <p class="fs-14 views mb-0">{{ $sliderPost->comment_count }}</p>
                                        </div>
                                    </div>
                                    <p class="description">
        ...
        <section>
            <div class="container">
                <div class="row">
                    <div class="col-12 col-md-8 home-first-left">
                        @foreach($headlinePosts as $row)
                            <div class="home-card position-relative mb-40">
                                ...
                                <div class="home-card-body">
                                    <h5 class="card-title mb-1 fs-16 text-black fw-6">
                                        <a href="{{route('detailPage',$row->slug)}}">{!! \Illuminate\Support\Str::limit(  $row->title,65,'...')  !!}</a>
                                    </h5>
                                    <p class="tag-and-date"><a href="{{route('categoryPage',$row->category->slug)}}"
                                          class="tags {{ getColorClass($row->category->id) }}">{!! $row->category->name !!}</a>
                                        <span
                                            class="date-text">{{  ucfirst(__('messages.common.'.strtolower($row->created_at->format('M')))) }} {{  $row->created_at->format('d, Y') }}</span>
                                    </p>
                    ...
                    <div class="col-12 col-md-4 side-menu">
                        @include('front_new.layouts.side-menu-part')
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="sub-section">
            <div class="container">
              <div class="row">
                  <div class="col-12 col-md-8 pr-40">
                      ...
                      @if($topAd)
                          ...
                          @else
                              <div class="container index-top-desktop">
                                  <a href="{{$adImageDesktopTop->ad_url}}"
                                     target="_blank">
                                      <img src="{{asset($adImageDesktopTop->ad_banner)}}"
                          ...
                          @else
                              <div class=" container index-top-mobile">
                                  <a href="{{$adImageMobileTop ->ad_url}}"
                                     target="_blank">
                                      <img src="{{asset($adImageMobileTop ->ad_banner)}}"
                              ...
                              <div class="tab-content">
                                  ...
                                  @foreach($postCategory as $category)
                                      ...
                                      @if(!empty($catePost->first()))
                                          <div
                                              class="tab-pane fade new-post pt-40 {{($loop->index == 0) ? 'show active' : ''}}"
                                              ...
                                              <div class="row">
                                                      ...
                                                      @foreach($catePost->where('visibility',\App\Models\Post::VISIBILITY_ACTIVE)->skip(1)->take(2) as $posts)
                                                          <div class="card mb-4 pb-sm-2 me-lg-0 pe-sm-1 me-sm-2">
                                                              <div class="card-img-top ">
                                                                  <a href="{{ route('detailPage',$posts->slug) }}">
                                                                      ...
                                                                      @elseif($posts->post_types == \App\Models\Post::VIDEO_TYPE_ACTIVE)
                                                                          ...
                                                                          <img
                                                                              src="{{(!empty($thumbUrl) ? $thumbUrl : $thumbImage)  }}"
                                                                              class="w-100 h-100" alt=""/>
                                                                      @else
                                                                          <img src="{{ $posts->post_image }}"
                                                                               class="w-100 h-100" alt=""/>
                                                              ...
                                                              <div class="card-body">
                                                                  ...
                                                                  <span
                                                                      class="card-text fs-12 text-gray">{{ ucfirst(__('messages.common.'.strtolower($posts->created_at->format('M')))) }} {{ $posts->created_at->format('d, Y')}}</span>
                                                              </div>
                                                          </div>
                                                      @endforeach
                                                  </div>
                                                  <div class="col-lg-6 mb-lg-0">
                                                      ...
                                                      @endphp
                                                      <div class="new-post-image position-relative">
                                          ...
                                          @foreach($category->posts->where('visibility',\App\Models\Post::VISIBILITY_ACTIVE) as $categoryPost)
                                              <div
                                                  ...
                                                  <div class="post-image position-relative">
                                                      ...
                                                      <div
                                                          class="post-content position-absolute px-30 mb-3 pb-1 w-100">
                                                          <h3 class="text-white pb-sm-2">
                                                              <a href="{{ route('detailPage',$categoryPost->slug) }}"
                      ...
                      @if($bottomAd)
                          ...
                          @else
                              <div class="container index-top-desktop">
                                  ...
                                  <a href="{{$adImageDesktopBottom->ad_url}}"
                                     target="_blank">
                                      <img src="{{asset($adImageDesktopBottom->ad_banner)}}"
                                           width="800" class="img-fluid">
                                  </a>
                              </div>
                          @endif
                          ...
                          @else
                              <div class=" container index-top-mobile">
                                  <a href="{{$adImageMobileBottom->ad_url}}"
                                     target="_blank">
                                                  ...
                                                  <div class="news-post-content">
                                                      ...
                                                      <div class="desc d-flex">
                                                          ...
                                                          <p class="fs-14 text-black mb-0"><a
                                                                  ...
                                                                  class="text-black">{{ __('messages.common.by') }}  {{ $latestPost->user->full_name }}</a>
                                                          </p>
                  ...
                  <div class="col-12 col-md-4 side-menu">
                      @include('front_new.layouts.side-menu-recommended')
                  </div>
              </div>
            </div>
        </section>
                                    ...
                                    @foreach($featurePostCategory as $category)
                                        <li class="nav-item pe-md-5 pe-4">
                                            <button
                                                ...
                                                data-bs-target="#menu-f-{{$category->id}}" type="button" role="tab"
                    ...
                    <div class="tab-content">
                        @foreach($featurePostCategory as $category)
                            ...
                            @if(!empty($featCatePost->first()))
                                <div
                                    class="tab-pane fade featured-post pt-60 {{($loop->index == 0) ? 'show active' : ''}}"
                    ...
                    <div class="col-xxl-6  col-lg-8 text-center">
                        <div class="blog">
                            ...
                            <div class="email-box position-relative">
                                ...
                                <input type="email" class="fs-14 text-gray subscribe-form" name="email" id="email_2"
                                       placeholder="{{ __('messages.common.your_email') }}" required>
                                <button type="submit"
